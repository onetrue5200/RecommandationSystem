...
class DataLoaderBase(object):
    ...
    def __init__(self, args, logging):
        ...
        self.train_file = os.path.join(self.data_dir, 'train_demo.txt')
        self.test_file = os.path.join(self.data_dir, 'test_demo.txt')
        self.kg_file = os.path.join(self.data_dir, 'kg_demo.txt')
        ...
        self.n_users, self.n_items, self.n_cf_train, self.n_cf_test = self.statistic_cf()

        if self.use_pretrain == 1:
            self.user_pre_embed, self.item_pre_embed = self.load_pretrained_data()

    def statistic_cf(self):
        n_users = max(max(self.cf_train_data[0]), max(self.cf_test_data[0])) + 1
        n_items = max(max(self.cf_train_data[1]), max(self.cf_test_data[1])) + 1
        n_cf_train = len(self.cf_train_data[0])
        n_cf_test = len(self.cf_test_data[0])
        return n_users, n_items, n_cf_train, n_cf_test
